...
def val(model, ref_model, t_model, t_ref_model, data_loader, tokenizer, device, block_num, t_test_transform):
    ...
    for images, text, ref_ids in metric_logger.log_every(data_loader, print_freq, header):
        ...
        image_embeds = t_model.visual_encoder(images) 
        ...
        output = t_model.text_encoder(text_input.input_ids, 
                                attention_mask = text_input.attention_mask, # torch.Size([batchsize, 6])
                                encoder_hidden_states = image_embeds,
                                encoder_attention_mask = image_atts,
                                return_dict = True,
                               )

        vl_embeddings = output.last_hidden_state[:,0,:] # torch.Size([batchsize, 768])
        vl_output = t_model.itm_head(vl_embeddings) # torch.Size([batchsize, 2])
        ...
        with torch.no_grad():
            ...
            grads = t_model.text_encoder.base_model.base_model.encoder.layer[block_num].crossattention.self.get_attn_gradients().detach()
            # torch.Size([b, 12, 6, 577])
            cams = t_model.text_encoder.base_model.base_model.encoder.layer[block_num].crossattention.self.get_attention_map().detach()
            # torch.Size([b, 12, 6, 577])

            cams = cams[:, :, :, 1:].reshape(images.size(0), 12, -1, 24, 24) * mask

            grads = grads[:, :, :, 1:].clamp(min=0).reshape(images.size(0), 12, -1, 24, 24) * mask
            # torch.Size([b, 12, 6, 24, 24])
...
def main(args, config):
    ...
    result = val(model, ref_model,  t_model, t_ref_model, test_loader, tokenizer, device, args.block_num, t_test_transform)
