...
from models.blip_model.blip_retrieval import BLIP_Retrieval, blip_retrieval
...
def retrieval_eval(args, model, ref_model, t_model, t_ref_model, t_test_transform, data_loader, tokenizer, t_tokenizer, device, config):
    model.float()      
    model.eval()
    ref_model.eval()
    t_model.float()
    ...
    img_attacker = ImageAttacker(images_normalize, eps=2/255, steps=10, step_size=0.5/255) # eps=2/255, steps=10, step_size=0.5/255
    ...
    for batch_idx, (images, texts_group, images_ids, text_ids_groups) in enumerate(data_loader): 
        ...
        with torch.no_grad():
            ...
            ### blip
            adv_texts_input = t_tokenizer(adv_texts, padding='max_length', truncation=True, max_length=30, return_tensors="pt").to(device)
            blip_text_ids.append(adv_texts_input.input_ids)
    ...
    s_score_matrix_i2t, s_score_matrix_t2i = retrieval_score(args, blip_text_ids, model, s_image_feats, s_image_embeds, s_text_feats,
                                                         s_text_embeds, s_text_atts, num_image, num_text, device=device)

    t_score_matrix_i2t, t_score_matrix_t2i = retrieval_score(args, blip_text_ids, t_model, t_image_feats, t_image_embeds, t_text_feats,
...
@torch.no_grad()
def retrieval_score(args, blip_text_ids, model, image_feats, image_embeds, text_feats, text_embeds, text_atts, num_image, num_text, device=None):
    if device is None:
    ...
    for i, sims in enumerate(metric_logger.log_every(sims_matrix, 50, header)): 
        ...
        if args.target_model == 'BLIP':
            text_ids = blip_text_ids
...
def eval_asr(model, ref_model, tokenizer, t_model, t_ref_model, t_tokenizer, t_test_transform, data_loader, device, args, config):
    ...
    score_i2t, score_t2i, t_score_i2t, t_score_t2i= retrieval_eval(args, model, ref_model, t_model, t_ref_model, t_test_transform,
...
# tcl_retrieval_checkpoint_flickr.pth
    ...
    # parser.add_argument('--target_model', default='ViT-B/16', type=str) # 'RN101'
    parser.add_argument('--target_model', default='BLIP', type=str) # 'RN101'
    ...
    parser.add_argument('--target_ckpt', default='./checkpoints/blip_retrieval_flickr.pth', type=str)    
